...
while ($row = oci_fetch_assoc($list11)){
    $d = $row["DOCNAME"];
    $c = $row["DOCEMAIL"];
...
if($_POST){
    ...
    $sqlmain = "SELECT * FROM doctor WHERE docemail=:keyword OR docname=:keyword OR docname LIKE :keywordStart OR docname LIKE :keywordEnd OR docname LIKE :keywordMiddle";
    $statement = oci_parse($connection, $sqlmain);
    oci_bind_by_name($statement, ":keyword", $keyword);
    oci_bind_by_name($statement, ":keywordStart", "$keyword%");
    oci_bind_by_name($statement, ":keywordEnd", "%$keyword");
    oci_bind_by_name($statement, ":keywordMiddle", "%$keyword%");
    oci_execute($statement);
}
else{
                ...
                <tr>
                   <td colspan="4">
                       <center>
                        ...
                        <?php
$result = oci_parse($connection, $sqlmain);
oci_execute($result);

if(oci_fetch_all($result, $rows) === 0){
    echo '<tr>
    <td colspan="4">
    <br><br><br><br>
    <center>
    ...
    foreach($rows as $row){
        ...
        $spcil_stmt = oci_parse($connection, "SELECT sname FROM specialties WHERE id=:spe");
        oci_bind_by_name($spcil_stmt, ":spe", $spe);
        oci_execute($spcil_stmt);
        $spcil_row = oci_fetch_assoc($spcil_stmt);
        $spcil_name = $spcil_row["SNAME"];
        ...
        if($action=='drop'){
            ...
            <div id="popup1" class="overlay">
                    ...
                    <center>
                        ...
                        Quieres borrar esto?<br>('.substr($nameget,0,40).').
